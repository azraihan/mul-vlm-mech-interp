...
representations do not emerge at any layer, confirming that meaningful visual
content is necessary to activate the pivot pathway.
...
\item We show that meaningful visual content is \emph{necessary} for language-specific
...
\begin{figure*}[t]
  ...
  \caption{
    ...
    Top row: real image; bottom row: all-white image.
...
\paragraph{All-white-image condition.}
Without meaningful visual content (image replaced by an all-white tensor of
...
This confirms that meaningful visual content is not merely incidental to the
pivot---it is \emph{necessary} to activate the representational pathway that
produces language-specific outputs.
...
ablation (\S\ref{sec:logit_lens}) provides positive evidence that meaningful
visual content shapes \emph{when} language-specific representations emerge at all.
...
\caption{
  ...
  ``Steer'' = best $\alpha$: $\alpha{=}0.2$ (real image), $\alpha{=}0.02$ (all-white image).
}
...
All-white image  & 42.0 & 41.5 & 46.5 & 48.5 \\
\bottomrule
\end{tabular}}
\end{table}
...
All-white images drastically lower baseline performance (PT: $-21$~pp;
RU: $-5$~pp), confirming that meaningful visual content is necessary for
correct answering.
With all-white images, steering is marginal or slightly harmful, consistent
with the logit-lens finding that meaningful visual content is required for
strong language-specific representations to form.
