...
all measurable pivot signal running through text-token positions.
Without visual context, language-specific representations do not emerge at any
layer, confirming that visual content is necessary to activate the pivot pathway.
...
Our results are consistent with the English pivot being a structural property
of the LLM backbone that multimodal pre-training does not mitigate;
we hypothesise that analogous pivot structures will be found in other
LLM-backbone VLMs, and propose our mechanistic methodology as a blueprint for
such investigations.
\end{abstract}
...
pivot in a multilingual VLM, conducting a focused case study on LLaVA-1.5-7B.
We view our findings as a case study rather than universal claims, and
hypothesise that analogous pivot structures---mediated through the LLM
backbone's concept space---will generalise to other VLMs built on
English-dominant LLM backbones; we propose our methodology as a blueprint
for testing this hypothesis in future work.
...
\item We show that visual context is \emph{necessary} for language-specific
  representations to emerge, and that under our patching design---which uses
  the same image in both passes---all measurable pivot signal runs through
...
residual-stream AIE profile peaks at layer~6 with a pivot region of
...
\begin{figure*}[t]
  ...
  \caption{
    ...
    Top row: real image; bottom row: all-black image.
...
\paragraph{All-black-image condition.}
Without meaningful visual content (image replaced by an all-black tensor of
the same spatial dimensions), PMI values remain near-zero or negative ($<{-}0.4$)
...
The grey pivot region (layers 5--17) identified in Figure~\ref{fig:logit_lens}
precedes the late-layer PMI peaks (20--29).
These two measurements are complementary, not contradictory:
the logit lens shows where representations \emph{crystallise} into decodable
token probabilities (a readout property), while causal patching
(\S\ref{sec:patching}) identifies where pivot information \emph{transfers}
between processing pathways (a causal property).
Causal English-pivot information is relayed through early-to-mid layers
and only decoded into language-specific probabilities later.
...
Figure~\ref{fig:visual} shows that the visual-token-only AIE is zero at every
layer across all languages.
This follows directly from the experimental design: the same image is used in
both the non-English (clean) and English (corrupted) passes, so the 576 CLIP
visual-token activations are identical and contribute no differential signal.
Accordingly, \emph{under this design}, all measurable causal pivot signal runs
through text-token positions.
Assessing whether visual tokens could carry independent pivot information would
require paired semantically-equivalent images in different languages---a
direction for future work.
What the current design \emph{does} establish is that the pivot's causal
pathway does not depend on any language-specific visual signal; the blank-image
ablation (\S\ref{sec:logit_lens}) provides the positive evidence that visual
context shapes \emph{when} language-specific representations emerge at all.
...
\midrule
Baseline              & 72.5 & 63.0 & 51.5 \\
Ours ($\alpha{=}0.2$) & 72.5 & \textbf{64.0} & \textbf{58.0} \\
\midrule
$\Delta$              & ---  & +1.0 & +6.5 \\
\bottomrule
\end{tabular}
...
at $\alpha{=}0.2$), substantially reducing the 21-point gap between Russian and
English baselines.
Portuguese gains +1.0~pp (63.0\% $\to$ 64.0\%).
We attribute the smaller Portuguese gain, at least in part, to a pivot-layer
mismatch: the residual-stream AIE profile for Russian spans approximately
layers 3--17, closely matching our chosen range (5--17), whereas the
Portuguese AIE profile peaks earlier and tapers off around layer~14
(Figure~\ref{fig:patching_heatmap}).
By using the shared range 5--17 we include three to four layers beyond
Portuguese's own pivot region---layers that carry little Portuguese pivot
signal but introduce noise into the steering intervention.
Using language-specific pivot layers (e.g.\ 5--14 for Portuguese) may yield
stronger gains; we treat this as a direction for future work.
...
\begin{figure*}[t]
  \centering
  \safefig[width=\textwidth]{../outputs/figures/pt_0015.png}%
...
\caption{
  ...
  ``Steer'' = best $\alpha$: $\alpha{=}0.2$ (real image), $\alpha{=}0.02$ (all-black image).
}
...
All-black image  & 42.0 & 41.5 & 46.5 & 48.5 \\
\bottomrule
\end{tabular}}
\end{table}
...
All-black images drastically lower baseline performance (PT: $-21$~pp;
RU: $-5$~pp), confirming visual context is necessary for correct answering.
With all-black images, steering is marginal or slightly harmful, consistent with
...
This work is a case study on a single VLM (LLaVA-1.5-7B) with three non-English
languages; we hypothesise that analogous pivot structures will be found in other
LLM-backbone VLMs, but generalisation to other architectures and language families
requires further investigation.
Our steering uses a shared pivot region (layers 5--17) derived from the
residual-stream AIE profile averaged across languages; however, language-specific
AIE profiles suggest that per-language pivot ranges may be narrower
(e.g.\ approximately 5--14 for Portuguese vs.\ 3--17 for Russian), and using
language-specific layer ranges is a promising direction for improving
steering gains, particularly for languages where the shared range is
over-inclusive.
...
In this case study of LLaVA-1.5-7B, we show that the model processes non-English
visual queries through an English-biased representational bottleneck in layers
5--17, with peak causal influence at layer~8 ($\Delta\mathrm{AIE}{=}0.49$).
...
backbone that multimodal pre-training does not mitigate, and provide a
mechanistically grounded methodology that we propose as a blueprint for
investigating pivot structures in other VLMs.
