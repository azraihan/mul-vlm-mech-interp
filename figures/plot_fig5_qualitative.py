...
def plot_curve(ax, en_curve, tl_curve, lang, title):
    ...
    if crossover is not None:
        ...
        ax.annotate(
            "",
            xy=(crossover, y_cross + arrow_len),
            xytext=(crossover, y_cross),
            arrowprops=dict(
                arrowstyle="-|>",
                color="red",
                lw=2.0,
...
def draw_figure(rows):
    """Render and save the 3-row qualitative figure from pre-computed curve rows."""
    n_rows = len(rows)
...
# ── JSON replay mode ──────────────────────────────────────────────────────────
# Usage: python figures/plot_fig5_qualitative.py <path/to/fig5_qualitative_data.json>
...
if len(sys.argv) > 1:
    ...
    pivot_layers = next(iter(all_data.values()))["pivot_layers"]

    rows = []
    for lang, d in all_data.items():
        img  = Image.open(d["image_path"]).convert("RGB")
        en_u = np.array(d["curves"]["unsteered"]["en"])
        tl_u = np.array(d["curves"]["unsteered"]["tl"])
        en_s = np.array(d["curves"]["steered"]["en"])
        tl_s = np.array(d["curves"]["steered"]["tl"])
        rows.append((lang, img, d["en_word"], d["tl_word"], en_u, tl_u, en_s, tl_s))
